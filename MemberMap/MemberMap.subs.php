...
function gmm_pinCount()
{
	...
	// Lets find the number of members that have placed their map pin for the template
	$request = $db->query('', '
		SELECT 
		    COUNT(*) as TOTAL
	...
	list($totalSet) = $request->fetch_row();
	$request->free_result();
...
function gmm_loadPins($loadAll = false)
{
	...
	{
		// More pins than allowed, show so load the data up at random to the number set in the admin panel
		$query = '
		SELECT 
			id_member
		FROM {db_prefix}members
